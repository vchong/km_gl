				...
				keymaster_blob_t *root_cert)
{
	...
	uint32_t key_attr_size = (RSA_KEY_BUFFER_SIZE +
			sizeof(uint32_t)) * KM_ATTR_COUNT_RSA;
	uint8_t *key_attr = TEE_Malloc(key_attr_size, TEE_MALLOC_FILL_ZERO);

	uint32_t output_certificate_size = ROOT_CERT_BUFFER_SIZE;
	uint8_t *output_certificate = TEE_Malloc(output_certificate_size,
			TEE_MALLOC_FILL_ZERO);

	//key-pair in format: size | buffer, ...
	uint32_t param_types = TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT,
			TEE_PARAM_TYPE_MEMREF_OUTPUT, //root RSA certificate
			TEE_PARAM_TYPE_NONE, //not used
			TEE_PARAM_TYPE_NONE); //not used
	TEE_Param params[TEE_NUM_PARAMS];
	...
	res = TA_serialize_rsa_keypair(key_attr, &key_attr_size, root_rsa_key);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to serialize root RSA attestation key, res=%x",
		     res);
		goto error_1;
	}

	///Link to command params
	params[0].memref.buffer = key_attr;
	params[0].memref.size = key_attr_size;

	params[1].memref.buffer = output_certificate;
	...
	//Invoke command
	res = TEE_InvokeTACommand(sessionSTA, TEE_TIMEOUT_INFINITE,
			CMD_ASN1_GEN_ROOT_RSA_CERT, param_types, params, NULL);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to invoke ASN.1 command GEN_ROOT_RSA_CERT, res=%x",
		     res);
		goto error_1;
	}

	root_cert->data_length = 0;
	if (params[1].memref.size == 0) {
		EMSG("ASN.1 GEN_ROOT_RSA_CERT output is empty");
		res = KM_ERROR_UNKNOWN_ERROR;
		goto error_1;
	}

	root_cert->data_length = params[1].memref.size;
	root_cert->data = TEE_Malloc(root_cert->data_length,
				     TEE_MALLOC_FILL_ZERO);
	if (!root_cert->data) {
		res = TEE_ERROR_OUT_OF_MEMORY;
		EMSG("Failed to allocate memory for root certificate output");
		goto error_1;
	}

	//Copy ASN.1 DER certificate
	TEE_MemMove(root_cert->data, params[1].memref.buffer,
			root_cert->data_length);
				...
				keymaster_blob_t *root_cert)
{
	...
	uint32_t key_attr_size = (EC_KEY_BUFFER_SIZE +
			sizeof(uint32_t)) * KM_ATTR_COUNT_EC;
	uint8_t *key_attr = TEE_Malloc(key_attr_size, TEE_MALLOC_FILL_ZERO);
	...
	//key-pair in format: size | buffer, ...
	uint32_t param_types = TEE_PARAM_TYPES(TEE_PARAM_TYPE_MEMREF_INPUT,
			TEE_PARAM_TYPE_MEMREF_OUTPUT, //root EC certificate
			TEE_PARAM_TYPE_NONE, //not used
			TEE_PARAM_TYPE_NONE); //not used
	TEE_Param params[TEE_NUM_PARAMS];
	...
	res = TA_serialize_ec_keypair(key_attr, &key_attr_size, root_ec_key);
	if (res != TEE_SUCCESS) {
	...
	//Invoke command
	res = TEE_InvokeTACommand(sessionSTA, TEE_TIMEOUT_INFINITE,
			CMD_ASN1_GEN_ROOT_EC_CERT, param_types, params, NULL);
	if (res != TEE_SUCCESS) {
		EMSG("Failed to invoke ASN.1 command GEN_ROOT_EC_CERT, res=%x", res);
		goto error_1;
	}

	root_cert->data_length = 0;
	if (params[1].memref.size == 0) {
		EMSG("ASN.1 GEN_ROOT_EC_CERT output is empty");
		res = KM_ERROR_UNKNOWN_ERROR;
		goto error_1;
	}

	root_cert->data_length = params[1].memref.size;
	root_cert->data = TEE_Malloc(root_cert->data_length, TEE_MALLOC_FILL_ZERO);
	if (!root_cert->data) {
		res = TEE_ERROR_OUT_OF_MEMORY;
		EMSG("Failed to allocate memory for root certificate output");
		goto error_1;
